...
function detectPoseInRealTime(video, net) {
  ...
  async function poseDetectionFrame() {
    ...
    poses.forEach(({score, keypoints}) => {
      ...
      var time = Date.now();
      var pose_data = {'score': score, 'keypoints': keypoints, 'time': time};
      g_pose_data_arr.push(pose_data);
...
function send_pose_data(){
  ...
  if(g_pose_data_arr.length>0){
    ...
    xmlHttp.open("post", backend_url + "/posedata"); 
    xmlHttp.send(formData); 
    g_pose_data_arr.length = 0;
  }
  else{
...
function startReport(time_interval) {
  var interval = setInterval(send_pose_data, time_interval);
}
